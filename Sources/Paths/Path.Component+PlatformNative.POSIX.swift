...
extension Path.Component {
    ...
    /// filesystem-native bytes (e.g., the code units returned by `readdir`,
    /// `FindFirstFileW`, or a `File.Name.rawBytes`). Consumer code can
    /// write a single unconditional call site instead of a `#if os(Windows)`
    /// hand-dispatch between a string-based init and a UTF-8-bytes-based init:
    ///
    /// ```swift
    /// // Before
    /// #if os(Windows)
